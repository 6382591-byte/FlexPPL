(function attachProgressModule(root, factory) {
  ...
  const volumeFor = (entry) => Number(entry.weight || 0) * (entry.sets || []).reduce((sum, reps) => sum + Number(reps || 0), 0);
  function estimatedOneRepMax(weight, reps) { const w=Number(weight),r=Math.max(...(reps||[]).map(Number).filter(Number.isFinite),0); return !w||!r||r>15?null:Math.round(w*(1+r/30)); }
  ...
  function personalRecords(history = []) {
    ...
    for(const entry of chronological){const id=entry.exerciseId,name=entry.displayNameAtTimeOfWorkout||entry.name,weight=Number(entry.weight||0),maxReps=Math.max(...(entry.sets||[]).map(Number).filter(Number.isFinite),0);
      ...
      const e1rm=estimatedOneRepMax(weight,entry.sets);if(e1rm&&e1rm>(bestOneRepMax.get(id)||0)){bestOneRepMax.set(id,e1rm);records.push({type:"e1rm",exerciseId:id,name,value:e1rm,date:entry.date,label:`Est. 1RM ${e1rm} lb`})}
